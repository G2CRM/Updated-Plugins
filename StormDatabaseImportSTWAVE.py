...
## This Code was modified 08/03/2021 to facilatate the import of H5 files regardless of ASCII vs UTF-8 string type
...
class StormDatabaseImportWave(QObject):
    ...
    def StartImport(self):
        ...
        dbfile = self.dbfile
        logger.debug("Trying to connect to %s", self.dbfile )
        ...
        for file in self.fileNames:
            ...
            sys.stdout.write("Importing STWAVE for Save Point {save} at ({lat}, {lon}) from file {file}\n".format(save=savePointID, lat=lat, lon=lon, file=file))
            ...
            for storm in h5File.list_nodes("/"):
                ...
                waveHeight = storm._f_get_child('Zero Moment Wave Height') # Wave 10/20/2015
                logger.debug("Wave Height: %s", waveHeight)
                waveDirection = storm._f_get_child('Mean Wave Direction') # Wave 10/20/2015
                logger.debug("Wave Direction: %s", waveDirection)
                wavePeriod = storm._f_get_child('Mean Wave Period') # Wave 10/20/2015
                logger.debug("Wave Period: %s", wavePeriod)
                ...
                timeStepCounter = -1
                timeMaxSurge = 0
                maxHeight = -9999999.0  # Wave 10/20/2015
                maxTimeStep = 0
                timeMaxHeight = 0  # Wave 10/20/2015
                minSurge = 9999999.0
                maxSurge = -999999.0
 ...
 ## Here @ 4:15 PM ##
                ...
                if (stormName not in stormNames):
                    logger.debug(f"Storm {stormName} not found in existing stormNames")
 ...
 ## Here @ 4:30 PM ##                                        
                ...
                if self.cursor.fetchone()[0] >= 1:
                    ...
                    for (date, elev, height, dir, period) in zip(dateData, elevData, waveHeight, waveDirection, wavePeriod):
                        ...
                        if (str(data) == 'nan' or data <= -9999.0):
                            ...
                            data += self.vDatumConv
                            

                        if (str(height).lower() == 'nan'):
                            height = None
                        else:
                            height *= 3.28084 # Convert to feet  Wave 10/20/2015
 ## Here @ 5PM ##    
                        try:
                            self.ImportStormDetail(summaryId, timeHours, timeStepCounter, data, height, period, dir)
                        ...
                        if (height > maxHeight):
                            maxHeight = height
                            timeMaxHeight = timeHours
                        if (data < minSurge):
                            minSurge = data
                    logger.debug("Importing Summary...")
                    self.ImportStormSummary(timeStepCounter, timeHours, minSurge, maxSurge, timeMaxSurge, stormNumber, summaryId, int(savePointID), maxHeight, timeMaxHeight)
 ...
 ## Here @ 5:15PM ##   
    ...
    def ImportStorms(self, stormnum, stormID, prob, type, emonth, eday, lmonth, lday, stormDate, stormDuration, stormActive, msstId, year):
        ...
        logger.debug(f"Queried for existing storm with ID {stormID}")
        if self.cursor.fetchone()[0] < 1:
    ...
    def ImportStormDetail(self, summaryId, timeHours, timeStepCounter, surge, height, period, dir):
        if height is None:
            height = 0
            period = 0
            dir = 0
            sys.stdout.write("WARNING: Invalid data while importing detail for storm/location {summary} at time {time}. Missing SignificantWaveHeight. Setting SignificantWaveHeight to 0.0\n".format(summary=summaryId, time=timeHours))

        elif float(height) < 0:
            sys.stdout.write("WARNING: Unexpected SignificantWaveHeight while importing detail for storm/location {summary} at time {time}. SignificantWaveHeight is {sigWave}.\n".format(summary=summaryId, time=timeHours, sigWave=height))

        SQL = "INSERT INTO ModeledStormLocationDetail (ModeledStormLocationID, TimeHours, TimeStepCounter, Surge, SignificantWaveHeight, WavePeriod, WaveDirection) VALUES (?, ?, ?, ?, ?, ?, ?)"
        self.cursor.execute(SQL, (summaryId, timeHours, timeStepCounter, surge, float(height), float(period), dir))

    def ImportStormSummary(self,  maxTimeStep, maxTimeHours, minSurge, maxSurge, timeMaxSurge, stormNum, summaryId, locId, maxHeight, maxHeightTime):
        SQL = "INSERT INTO ModeledStormLocationSummary (ModeledStormLocationID, StormNumber, MaximumNumberOfTimeSteps, MaximumTimeHours, MinimumSurge, MaximumSurge, TimeOfPeakSurge, LocationID, MaximumWaveHeight, TimeOfPeakWaveHeight) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        self.cursor.execute(SQL, (summaryId, stormNum, maxTimeStep, maxTimeHours, minSurge, maxSurge, timeMaxSurge, locId, maxHeight, maxHeightTime))